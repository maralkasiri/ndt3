...
df_paths = [
    Path("./data/eval_metrics/crc_neural_cst_neural_eval_ndt3.csv"),
    Path("./data/eval_metrics/crc_cst_neural_eval_ndt3.csv"),
]
...
target_df['marker_style'] = target_df.variant_stem.apply(marker_style_map)
...
def make_plots(
    ...
    if target_eval_set in y_min:
        ...
        upper = ylims[1] if target_eval_set not in y_max else y_max[target_eval_set]
...
def format_ax(df,
    ...
    if do_ylabel:
        ...
        elif metric == 'eval_neural_loss':
            ax.text(-0.1, 0.92, 'Neural\nNLL', ha='right', va='center', transform=ax.transAxes, fontsize=20)
