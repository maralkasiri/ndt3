...
parser.add_argument(
    "--eval-set", "-e", type=str, required=True, choices=['cursor_hi']
)
parser.add_argument(
    "--workers", "-w", type=int, default=0, help="Number of parallel workers to use for single GPU eval"
)
parser.add_argument(
    "--experiment", "-x", type=str, required=True, choices=['intra_session'], help="Experiment to evaluate"
)
parser.add_argument(
...
SCALES = args.scales
FILTER_BEST = False # we decided we actually don't want to reduce, we want to scatter all to build confidence for accuracy on line narrative

EXPERIMENT_MAP = {
    "intra_session": "v5/analyze/intra_session",
}
...
eval_metrics_path = eval_paths / f"intra_session.csv"
...
def get_run_df_for_query(variant: str, experiment_set: str, eval_set: str, metric='val_kinematic_r2', **kwargs):
    runs = get_runs_for_query(variant, experiment_set, **kwargs)
...
def get_eval_df(queries):
    ...
    for query in queries:
        query_dfs.append(get_run_df_for_query(query, EXPERIMENT_SET, EVAL_SET))
...
def process_row(index: int, run_row_id: str, run_row_eval_set: str):
    ...
    return get_single_eval(cfg, src_model, dataset=dataset, device=device)
...
def exec_eval():
    ...
    eval_df = eval_df.drop_duplicates(subset=['variant', 'seed', 'model.lr_init', 'eval_set','experiment_set'], keep='first').reset_index(drop=True)
