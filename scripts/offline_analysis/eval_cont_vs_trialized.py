...
parser.add_argument(
    "--eval-set", "-e", type=str, required=True, choices=['cursor_cont', 'cursor_trialized']
)
parser.add_argument(
    "--workers", "-w", type=int, default=0, help="Number of parallel workers to use for single GPU eval"
)
parser.add_argument(
    "--experiment", "-x", type=str, required=True, choices=['cursor_trialized', 'cursor_cont'], help="Experiment to evaluate"
)
parser.add_argument(
...
EXPERIMENT_SET = args.experiment
...
EXPERIMENT_MAP = {
    "cursor_trialized": "v5/analyze/cursor_trialized",
    "cursor_cont": "v5/tune/cursor",
}
...
EVAL_DATASET_FUNC_MAP = {
    'cursor_cont': 'eval_pitt_eval_broad.*',
    'cursor_trialized': 'eval_pitt_trialized_broad.*',
}

eval_paths = Path('~/projects/ndt3/data/analysis_metrics').expanduser()
eval_paths.mkdir(parents=True, exist_ok=True)
eval_metrics_path = eval_paths / f"cont_trialized.csv"
...
def get_runs_for_query(variant: str, scale_ratio: float, experiment_set: str, project="ndt3", exp_map=EXPERIMENT_MAP):
    r"""
...
def run_list_to_df(runs, eval_set: str, experiment_set: str):
    ...
    # remove runs that have "smoketest" in tag
    filter_runs = [r for r in filter_runs if 'smoketest' not in r.config['tag']]
    ...
    # Get the precise eval metric associated with this ckpt
...
def get_run_df_for_query(variant: str, scale_ratio: float, experiment_set: str, eval_set: str, metric='val_kinematic_r2', **kwargs):
    runs = get_runs_for_query(variant, scale_ratio, experiment_set, **kwargs)
    run_df = run_list_to_df(runs, eval_set=eval_set, experiment_set=experiment_set)
...
def get_eval_df(queries):
    ...
    for query in queries:
        query_dfs.extend([get_run_df_for_query(query, scale_ratio, EXPERIMENT_SET, EVAL_SET) for scale_ratio in scales])
...
def get_single_eval(cfg: RootConfig, src_model, dataset, stream=False, device=torch.device('cuda')):
    pl.seed_everything(0)
    dataset = SpikingDataset(cfg.dataset, use_augment=False)
    dataset.cfg.max_tokens = 32768
    dataset.cfg.max_trial_length = 1500
...
def process_row(index: int, run_row_id: str, run_row_eval_set: str):
    ...
    try:
        ...
        return 0
    assert cfg.dataset.eval_ratio == 1.0, "Expecting full eval set"
    cfg.dataset.eval_datasets = [eval_set]
    cfg.dataset.datasets = cfg.dataset.eval_datasets # Ensure alignment
    # Avoid reprocessing dataset with run's dataset config
    # breakpoint()
    if run_row_eval_set == 'cursor_trialized':
        cfg.dataset.pitt_co.respect_trial_boundaries = True
        cfg.dataset.pitt_co.chop_size_ms = 0
    elif run_row_eval_set == 'cursor_cont':
        cfg.dataset.pitt_co.respect_trial_boundaries = False
        cfg.dataset.pitt_co.chop_size_ms = 1000
    else:
        raise ValueError(f"Unknown eval set: {run_row_eval_set}")
    dataset = SpikingDataset(cfg.dataset, use_augment=False)
    dataset.subset_split(splits=['eval'])
    ...
    return get_single_eval(cfg, src_model, dataset=dataset, device=device, stream='cont' in run_row_eval_set)
...
def exec_eval():
    ...
    eval_df = eval_df.drop_duplicates(subset=['variant', 'seed','eval_set','experiment_set'], keep='first').reset_index(drop=True)
