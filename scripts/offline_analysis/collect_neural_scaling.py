#%%
# Save down neurometrics from eval report (don't recompute)
# Intended to just be used for neural cst
...
parser.add_argument(
    "--eval-set", "-e", type=str, required=True, choices=['neural_cst', 'cst'],
)
parser.add_argument(
...
eval_metrics_path = eval_paths / f"{get_simple_host()}_{EVAL_SET}_neural_eval_ndt3.csv"
...
def run_list_to_df(runs, eval_set_name: str):
    ...
    df_dict = {
        ...
        'val_neural_loss': map(lambda r: r.summary['val_spike_infill_loss'], filter_runs),
    ...
    eval_reports_neural = [
        rh.loc[rh['val_spike_infill_loss'].idxmin()]['eval_spike_infill_loss'] for rh in run_histories
    ]
    df_dict['eval_neural_loss'] = eval_reports_neural
