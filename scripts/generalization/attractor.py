...
if 'ipykernel' in sys.modules:
    ...
    parser.add_argument(
        "--eval-set", "-e", type=str, required=True, choices=['emg_co', 'vel_co', 'rtt']
    )
    parser.add_argument(
...
def get_single_eval(cfg: RootConfig, src_model, dataset, device=torch.device('cuda')):
    ...
    for batch in dataloader:
        ...
        out_unflat[Output.pseudo_trial] = batch['covariate_length']
        batch_outputs.append(out_unflat)
    # We've unflattened across kin dims but not across trials
...
def process_row(
    ...
    try:
        ...
        print(f"Failed to load run {run_id}, probably missing ckpt")
        return 0
    is_wedge_held_in = held_out[0] in held_in
    ...
    if np.isclose(eval_r2, 0) or np.isclose(eval_r2, 1):
        ...
        print(f"Warning: {run_id} eval_r2 is {eval_r2}")
        breakpoint()
...
def exec_eval():
    ...
    if PARALLEL_WORKERS:
        ...
        with Pool(processes=PARALLEL_WORKERS) as pool:  # Adjust pool size if needed
            results = pool.map(process_row_wrapper, eval_df.itertuples(index=False, name=None))
    else:
    ...
    eval_df = eval_df.drop_duplicates(subset=[
        'variant', 'experiment', 'held_in', 'held_out',  # TODO phase
