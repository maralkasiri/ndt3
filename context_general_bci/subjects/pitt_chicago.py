...
r"""
    ...
    Referenced from array locs in https://github.com/pitt-rnel/bci_analysis
    - And note on https://github.com/pitt-rnel/bci_analysis/blob/b8c28d635f3e820b1fcec2dd7fb7b91dd00f7902/CMG/Utils/Misc/LoadSubjectChannelMap.m
...
@SubjectArrayRegistry.register(other_aliases=['P2Lab', 'P2Home']) # Note, PTest proxied into P2 for ease. Not actually real data.
...
class BCI02(SubjectInfoPittChicago):
    ...
    blacklist_channels = np.concatenate([
        np.arange(1, 129), # 1st array
        np.arange(65, 97) # Stim channel
    ])

    blacklist_pedestals = np.concatenate([
        np.zeros(128, dtype=int),
...
@SubjectArrayRegistry.register
class BCI03(SubjectInfoPittChicago):
    # Identical to BCI03
    # Layout shared across motor channels
    name = SubjectName.BCI03
    _motor_layout = np.array([ # wire bundle to right, viewing from pad side (electrodes down)
        [np.nan, 38, 50, 59,  6, 23,  22, 101, 111, np.nan,],
            [33, 40, 46, 64,  9, 25,  24, 102, 113, 128],
            [35, 43, 56, 61, 17, 21,  26, 103, 112, 114],
    ...
    @classmethod
    @property # Order flipped for BCI03
    def arrays(cls):
        return {
            'lateral_s1': cls.sensory_arrays[1],
            'medial_s1': cls.sensory_arrays[0],
