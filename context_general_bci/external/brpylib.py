...
Lightly patched: https://github.com/BlackrockNeurotech/Python-Utilities/blob/main/brpylib/brpylib.py
- The public release is pretty incomplete, we add a few minimal features for our use case inc. subset electrode reading.
...
from context_general_bci.external.brMiscFxns  import openfilecheck, brmiscfxns_ver
...
######
        ...
        if len(commentPackets) > 0 :
            ...
            trueComments = np.setdiff1d(list(range(0,len(commentPackets)-1)),ROIPackets).astype(bool)
...
class NsxFile:
    ...
    def getdata(self, elec_ids='all', start_time_s=0, data_time_s='all', downsample=1, zeropad=False):
        ...
        # JY: len of data_headers indicate number of discrete data chunks. In most cases we are hoping for 1. JY only developed for 1 as happy path. 
        # JY did not test zero pad path.
        # breakpoint()
        for x in range(len(output['data_headers'])) :
            ...
            try:   np.empty((data_length, num_elecs), dtype=np.short) # mini opt skip setting
            ...
            if elec_ids == ELEC_ID_DEF:
                recorded_data_bytes = data_length * num_elecs * DATA_BYTE_SIZE; # JY comment to enable proper subsetting
                ...
                recorded_payload = np.ndarray((data_length, num_elecs),
                                             '<h',
                                             recorded_data)
            else:
                # JY monkey patch. Assume within each data chunk that the data is contiguous in terms of elecs. Instead of simply taking the first num-elecs worth of bytes, seek to the correct indices...
                # Ignores paging mxsms implemented in subsetnsx - JY is not sure when exactly that will trigger, JY's source files are larger than 1G and run fine without obv appearance of multiple pages.
                shape = (data_length, self.basic_header['ChannelCount'])
                file_offset = d_ptr['BoD'][x]
                mm = np.memmap(self.datafile, dtype=np.int16, mode='r', offset=file_offset, shape=shape)
                if elec_id_indices: mm = mm[:, elec_id_indices]
                recorded_payload = mm
                assert not zeropad, 'not implemented'
            output['data'].append(recorded_payload)
        ...
        # * JY: Monkey patch if statements assuming that the same ops/scaling should be done regardless of # of data chunks there are
        if isinstance(output['data'], list):
            output['data'] = [entry.T for entry in output['data']]
        else:
        ...
        if self.basic_header['FileSpec'] == '2.1':  output['data'] *= UV_PER_BIT_21
            ...
            if front_end_idxs:
                if front_end_idx_cont:
                    transform = lambda data: data * getdigfactor(self.extended_headers, output['ExtendedHeaderIndices'][front_end_idxs[0]])
                    if isinstance(output['data'], list):
                        for i, data in enumerate(output['data']):
                            output['data'][i] = data.copy()
                            output['data'][i][front_end_idxs[0]:front_end_idxs[-1] + 1] = transform(
                                data[front_end_idxs[0]:front_end_idxs[-1] + 1]
                            )
                    else:
                    ...
                    if isinstance(output['data'], list):
                        for i, data in enumerate(output['data']):
                            output['data'][i] = data.copy()
                            for j in front_end_idxs:
                                output['data'][i][j] *= getdigfactor(self.extended_headers, output['ExtendedHeaderIndices'][j])
                    else:
            ...
            if analog_input_idxs:
                if analog_input_idx_cont:
                    transform = lambda data: data * getdigfactor(self.extended_headers, output['ExtendedHeaderIndices'][analog_input_idxs[0]])
                    if isinstance(output['data'], list):
                        for i, data in enumerate(output['data']):
                            output['data'][i] = data.copy()
                            output['data'][i][analog_input_idxs[0]:analog_input_idxs[-1] + 1] = transform(
                                data[analog_input_idxs[0]:analog_input_idxs[-1] + 1]
                            )
                    else:
